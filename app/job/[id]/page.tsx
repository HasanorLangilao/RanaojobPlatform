    ...
    const fetchJobData = async () => {
      try {
        ...
        if (jobData.employerId) {
          try {
            // Use employer data from job document if available
            if (jobData.employerRating !== undefined) {
              employerRating = jobData.employerRating;
              employerReviewCount = jobData.employerReviewCount || 0;
              actualAverageRating = jobData.actualAverageRating || 0;
            } else {
              // Get employer rating directly from the employer document
              // This is faster than fetching all reviews
              ...
              if (employerDoc.exists()) {
                ...
                actualAverageRating = employerData.actualAverageRating || 0;
              }
            }
          } catch (error) {
        ...
        setJob({
          ...
          actualAverageRating: actualAverageRating,
        })
                  ...
                  {/* <div className="flex items-center">
                    {Array(5).fill(0).map((_, i) => (
                      <Star
                        key={i}
                        size={16}
                        className={`${i < Math.round(job.employerRating) ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'}`}
                      />
                    ))}
                  </div> */}
                  {/* <span className="text-sm text-yellow-500 ml-1">{job.employerRating.toFixed(1)}</span>
